 ...
 * differing content overwrites the existing entry and logs a warning.
 *
 * Run this once after standing up a fresh KB (or after wiping the KB's
 * state directory). Skills that use a specific schema (`tag-irac`,
 * `subsequent-treatment`) also self-register the schemas they need —
 * this skill exists for the bootstrap case where the user wants the
 * full schema set available to the TaggingPanel UI before running any
 * specific skill.
 *
...
async function main(): Promise<void> {
  ...
  const kb: KnowledgeBase = {
    id: 'caselaw-register-tag-schemas',
    label: 'caselaw register-tag-schemas',
    email,
